...
public class ChatService {
    ...
    private static final String SYSTEM_PROMPT_TEMPLATE = """
            ...
            - 加购标签格式：[ADD_TO_CART:商品ID:数量:规格标签]
              · 数量**带 +** 表示「累加」：+1 = 加购1件、+2 = 再加2件
              · 数量**不带 +** 表示「设为指定数量」：3 = 设为3件（覆盖）
            - 标签中必须带上用户选择的规格标签（与商品库中"规格可选"一致的文字）
            - 示例：
               - [ADD_TO_CART:p_digital_007:+1:42码]  → 加购 42码 ×1（累加）
               - [ADD_TO_CART:p_beauty_001:+1:50ml 加大装]  → 加购 50ml ×1（累加）
               - [ADD_TO_CART:p_digital_007:3:42码]  → 设为 42码 ×3（覆盖）
               - [ADD_TO_CART:p_digital_007:+2:42码]  → 再加 2 件 42码（累加）
            - 语义判断：
               - "加入购物车"、"加购"、"买一个" → 先问规格，确认后 [ADD_TO_CART:id:+1:规格]
               - "要三台"、"设为 5 个" → [ADD_TO_CART:id:3:规格]
               - "再来两台"、"再加 2 个" → [ADD_TO_CART:id:+2:规格]
            - 用户回复数字编号时，根据第二步展示的编号列表对应规格

            ❌ 错误：[ADD_TO_CART:p_001]（没带规格，会加成默认标准）
            ✅ 正确：[ADD_TO_CART:p_001:+1:40码]（带上规格；+1 表示加购1件）
