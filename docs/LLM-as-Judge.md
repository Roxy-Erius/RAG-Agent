...
## 12. 预算感知检索 + rubric/prompt 迭代（实测记录）

评测暴露出「用户给了预算、agent 却推荐超预算商品」的体验问题。围绕它做了三轮迭代：

### 12.1 预算感知检索（方案 B：仅标注，不重排）
- `RetrieverService.retrieveProductsMultiModal(query, topK, category, maxPrice)`：**相关性优先**，只对结果标注 `overBudget`
- `ProductSearchResult` 加 `overBudget` 字段；`ChatService.parseBudget()` 正则解析预算；上下文里超预算商品带 `⚠️超预算`
- prompt 规则 6/7：优先预算内；无匹配时**明确告知超出多少**、询问是否放宽，禁止"略超预算"淡化

**踩坑**：最初做成"严格按预算过滤 + 超预算补齐"，结果**把语义最相关但超预算的商品滤掉了**（跑鞋全 >800），只剩"便宜但不相关"的 T 恤 → 相关/忠实暴跌。
**结论**：**相关性优先，预算只做标注**，别让价格过滤压过相关性。

### 12.2 rubric 修正（方案 A：维度正交）
- 忠实度**只衡量有无编造**：合理追问且未编造 → 5
- 相关性改衡量「是否恰当回应任务」：合理追问、无匹配如实告知都算相关；该推荐却无谓追问才低分
- 效果：相关 4.40→4.73，忠实 3.73→4.27

### 12.3 prompt 迭代（失败 → 成功）
- **失败版**：加"必须直接推荐、禁止追问" → 修好了"无谓追问"，但**逼 agent 在无匹配时硬推**（洗发水 5→1），整体变差 → **回滚**
- **成功版**：改成"**优先**直接推荐 + 无匹配则如实告知"（不覆盖"无匹配不编造"规则）→ 相关升到 **4.93**，背包修好、洗发水不回归
- **教训**：**"强制推荐"这类一刀切规则很危险**——它假设"总有匹配商品"，一旦没有就会诱发硬推/编造。

### 12.4 判官噪声
语气维度同一风格回复在不同轮给 1/2/5，**波动极大**（3B 判官通病）。看**相对趋势**，别迷信单条绝对分。
